...
\begin{frame}{Distance Method without Ground Truth}
    ...
    \begin{normalblock}{Method steps}
        \begin{itemize}
            ...
            \item Latent representations $\mathcal{S}_{train} = {\{\mathbf{s}_i\}}_{i=1}^{n_1}$ of $\mathcal{D}_{train}$ extracted.
            \item \textcolor{red}{Clusters $\mathcal{C} = {\{c_j\}}_{j=1}^{|\mathcal{K}|}$ formed using clustering algorithms.}
            \item Centroids $\mathbf{\mu}_j$ found for each cluster.
            \item Distances $d_{i,j}$ from centroids evaluated.
        \end{itemize}
  \end{normalblock}
\end{frame}
